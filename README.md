...
Start the server using below line and open http://localhost:8000 in your browser.
...
Detailed rationale for each decision is in `docs/DESIGN_DECISIONS.md`. Short version:
...
**Schema-first extraction:** The LLM infers table columns before extraction, not after. This was based on reading the WebLists paper -- they found that LLMs with search get 3% recall on structured extraction without a schema, vs 66% with schema-bound extraction.

**Constrained JSON output:** We use Groq's `json_object` response format mode instead of asking the LLM to return JSON in a prompt. This guarantees valid JSON syntax. Note: `llama-3.3-70b-versatile` supports `json_object` mode (valid JSON guaranteed) but not `json_schema` mode (schema-level constrained decoding), which is currently limited to GPT-OSS models on Groq.
...
**HTML cleaning:** We strip nav/footer/ad elements before sending content to the LLM. This is the web equivalent of stopword removal -- reduces noise so the extractor sees actual content.
...
- **Rate limiting** on Groq's free tier causes 429 errors with retry delays. A typical query takes 60-180 seconds, mostly waiting on LLM calls and retries.
- **Entity deduplication** uses string matching. "Alphabet" and "Google" would not be recognized as the same entity. Embedding-based similarity would improve this.
